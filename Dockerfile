...
RUN apt-get update 
# Doing this so it's cached... 
# libvbx-dev doesn't exist.
RUN apt-get install -y curl yasm vim g++ git automake frei0r-plugins-dev ladspa-sdk libladspa-ocaml-dev libiec61883-dev libass-dev libbluray-dev libbs2b-dev \
        ...
        libopenjp2-7-dev libpulse-dev libspeex-dev libvorbis-dev libx264-dev 

RUN git clone https://git.ffmpeg.org/ffmpeg.git ffmpeg

#RUN git clone https://github.com/webmproject/libvpx.git libvpx
#RUN cd libvpx; git checkout v1.6.1; ./configure && make && make install

# Not using libvpx... it requires /\ but that doesn't compile it seems
RUN cd ffmpeg; ./configure --prefix=/usr --extra-version=ubuntu-17.10.1 --build-suffix=-ffmpeg --toolchain=hardened --libdir=/usr/lib/x86_64-linux-gnu --incdir=/usr/include/x86_64-linux-gnu --enable-gpl --enable-shared --disable-stripping --enable-avresample --enable-avisynth --enable-frei0r --enable-gnutls --enable-ladspa --enable-libass --enable-libbluray --enable-libbs2b --enable-libcaca --enable-libcdio --enable-libfontconfig --enable-libfreetype --enable-libfribidi --enable-libgme --enable-libgsm --enable-libmodplug --enable-libmp3lame --enable-libopenjpeg --enable-openal --enable-libopus --enable-libpulse --enable-librtmp --enable-libshine --enable-libspeex --enable-libtwolame --enable-libvorbis --enable-libwavpack --enable-libwebp --enable-libxvid --enable-libzvbi --enable-opengl --enable-libdc1394 --enable-libzmq --enable-libssh --enable-libsoxr --enable-libx264 --enable-nvenc --enable-nonfree \
        && make \
        && make install
